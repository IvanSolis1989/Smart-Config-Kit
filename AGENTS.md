...
另有间接覆盖的客户端不单独出产物：
- **Hiddify**：内核即 sing-box，消费 `SingBox/singbox-smart*.json`；见 `SingBox/使用教程.md §2a`
...
3. **同步更新每个产物头部的「介绍 / 更新日志 / 版本号」注释块**（详见 `CLAUDE.md §1.3`）——此处是本仓库追踪历史的唯一权威来源，版本号必须与 Clash Party 主线对齐，"本次变更"至少新增 1 行，PR 描述仅作辅助。
4. **同步更新** `README.md` + 对应 `使用方法.md` / `使用教程.md`
5. **跑完 `CLAUDE.md` §5 自检命令**，结果附在 PR 描述里
...
- [ ] **每个被动过的产物头部注释块「介绍 / 更新日志 / 版本号」已同步更新**（至少 bump 尾段版本号 + 新增 1 行变更说明）
...
| 改代码但忘了 bump 头部注释的版本号/更新日志 | 多次 | 历史追溯失效，后续代理无法判断当前主线是哪个版本 | 每次修改必须 bump 尾段版本 + 新增 1 行变更说明 |
| OpenClash full 头部注释缺少详细介绍/变更日志 | 当前 | 用户无法知道此文件与 slim 的差异、与 Clash Party 的基线对齐时间 | 参照 slim 的头部块格式补齐 |
